...
class ExternalResourceRepositoryAdapter[F[_]: Monad](
  ...
  def bulkUpsert(data: List[ExternalResource]): ContT[F, Int, Int] = {
  ...
  def findByContentId(contenId: ContentId): ContT[F, Seq[ExternalResource], Seq[ExternalResource]] = {
  ...
  def bulkDelete(data: List[ExternalResourceDeleteModel]): ContT[F, Unit, Unit] = {
    data.size match {
