...
import net.yoshinorin.qualtet.domains.{FeedsPagination, Limit, Page, Pagination, PaginationQueryParametersModel, PaginationQueryParametersOps}
...
class FeedRoute[F[_]: Concurrent, G[_]: Monad @nowarn](
  ...
  feedPaginationOps: PaginationQueryParametersOps[FeedsPagination],
  config: FeedConfig
  ...
  private val pagination = feedPaginationOps.make(PaginationQueryParametersModel(Option(Page(1)), Option(Limit(config.limit)), None))

  private[http] def index: HttpRoutes[F] = HttpRoutes.of[F] { implicit r =>
    (r match {
      case request @ GET -> Root / name => this.get(name, pagination)
  ...
  private[http] def get(@nowarn name: String, pagination: Pagination): Request[F] ?=> F[Response[F]] = {
    (for {
      feeds <- EitherT(feedService.get(pagination))
